...
from gsets import gset_as_dict, connection_matrix_gsets
#from embedding import get_umap
...
def plot_distribution(df, groups, labs, up_feat, dw_feat=None, save_file=None):
    ...
    for j,uf in enumerate(up_feat):
        ...
        ax[-1].set_xticklabels([min_x,(max_x-min_x)/2+min_x,max_x], fontsize=15)
        ...
        if dw_feat is not None and len(dw_feat)>j:
            ...
            ax2.set_xticklabels([min_x,(max_x-min_x)/2+min_x,max_x], fontsize=15)
...
def plot_genes_network(gset, subsel, exp=None, cutoff=.1, save_file=None):
    ...
    sets_dict = gset_as_dict(subsel=subsel)
    gset      = sets_dict[gset]
